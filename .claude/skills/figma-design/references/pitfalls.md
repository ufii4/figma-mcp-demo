...
**Solution**: Pass `layoutSizingHorizontal: "FILL"` directly in `create_text`:
```
create_text(text: "Long paragraph...", parentId: frameId,
  layoutSizingHorizontal: "FILL")
```
No separate `set_layout_sizing` call needed.
...
**Solution**: After creating a style, apply it to nodes:
```
# For text, apply at creation time:
create_text(text: "Title", parentId: frameId, textStyleId: headingStyleId)

# For existing nodes:
apply_style_to_node(nodeId, styleId, styleType: "fill")    <- paint style
apply_style_to_node(nodeId, styleId, styleType: "text")    <- text style
apply_style_to_node(nodeId, styleId, styleType: "effect")  <- effect style
```
